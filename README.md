...
2. **参照テキストを入力** — その音声で実際に話している内容。「自動文字起こし（Whisper）」ボタンで書き起こさせることもできます
3. **読み上げるテキストを入力** → 「生成する」
4. 必要なら「再生速度」を調整。生成後に「速度だけ変えて出力し直す」を押せば、TTS を再実行せずに速度だけ変えられます
5. 生成音声は再生・ダウンロードできるほか、`outputs/` に日時つき WAV（24kHz）で自動保存されます。ダウンロードされるファイルは速度を適用した後の音声です（等倍以外はファイル名に `_x0.90` のような接尾辞が付きます）
...
| --- | --- | --- |
| 言語 | 自動判定 | 上記 10 言語に対応 |
| 再生速度 | 1.0 | 0.5〜2.0 倍。声の高さは変えずに話速だけ変わる |
...
再生速度は Qwen3-TTS 自体が話速指定に対応していないため、生成後の波形処理（WSOLA）で伸縮しています。単純なリサンプリングと違って声の高さは変わりません。0.75〜1.25 倍で実測したところ、長さの誤差は 2% 以内、基本周波数の変化は 0.5% 以内でした。
